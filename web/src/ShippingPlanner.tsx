...
export default function ShippingPlanner({ go }: Props) {
  ...
  useEffect(() => {
    ...
    const setupMap = async () => {
      ...
      const pickup = await getShippingPickup().catch(() => ({ address: 'B & B Auto Exchange dispatch location', latitude: 39.5, longitude: -98.35 }))
      const L = leaflet()
  ...
  return <main className="hollywood-shipping">
    ...
    <section className="shipping-route wrap-wide">
      <div className="shipping-map-wrap"><div className="shipping-map-container" ref={mapNode} aria-label="Interactive delivery map" /><p className="shipping-map-tip">The pickup point is fixed by B & B Auto Exchange. Click anywhere on the map to set or replace the delivery destination.</p></div>
    ...
    <section className="shipping-protection"><div className="shipping-protection-inner wrap-wide"><header><div><p className="garage-kicker">THE B &amp; B TRANSPORT STANDARD</p><h2>CLASSIC CARE,<br /><em>ALL THE WAY.</em></h2></div><p>From the moment your vehicle leaves B &amp; B Auto Exchange to its arrival at your preferred address, every detail is planned around protection, clarity, and confidence.</p></header><div className="shipping-protection-grid"><article><span>01</span><h3>ENCLOSED VEHICLE<br />TRANSPORT</h3><p>Your vehicle travels in an enclosed carrier, shielded from weather, road debris, and outside exposure throughout the journey.</p></article><article><span>02</span><h3>RELIABLE DOMESTIC<br />SHIPPING</h3><p>We arrange secure delivery across the United States, bringing your classic car directly to the location you choose.</p></article><article><span>03</span><h3>FULLY INSURED<br />TRANSPORTATION</h3><p>Your vehicle is fully insured while in transit, adding protection and peace of mind from pickup through delivery.</p></article></div></div></section>
