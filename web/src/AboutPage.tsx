...
const fallback: AboutContent = {
  eyebrow: 'Who we are',
  title: 'ABOUT B & B AUTO EXCHANGE',
  intro: 'B & B Auto Exchange is dedicated exclusively to retro and classic vehicles, with clear information and a personal buying experience.',
  story: {
    title: 'Our Story',
    paragraphs: [
    ...
    imageCaption: 'The B & B standard: timeless vehicles and clear details.',
  ...
  contact: {
    title: 'Contact & Location', addressLabel: 'Showroom address', address: 'Contact B & B Auto Exchange for showroom details and appointments.', phoneLabel: 'Phone', phone: 'Call us to discuss a vehicle', phoneDetail: 'Appointments and calls are coordinated directly with the B & B team.', emailLabel: 'Email', email: 'sales@bbautoexchange.com', emailDetail: 'We respond as soon as possible during business hours.', hoursLabel: 'Business hours', hours: 'By appointment / Monday-Saturday',
  },
  stats: [{ value: 'Retro only', label: 'Vehicle selection', detail: 'Classic vehicles with character' }, { value: 'Nationwide', label: 'Delivery planning', detail: 'Across the United States' }, { value: 'Direct', label: 'Personal support', detail: 'Clear answers at every step' }, { value: 'Detailed', label: 'Vehicle information', detail: 'Condition and history context' }],
}
...
export default function AboutPage({ go }: Props) {
  ...
  if (!content) return <section className="about-loading">Loading the B & B story...</section>
  ...
  const hasMap = !address.toLowerCase().startsWith('contact b & b')
  ...
  return <main className="hollywood-about">
    ...
    <section className="about-main wrap-wide">
      <article className="about-story"><h2>{story.title}</h2>{story.paragraphs.map((paragraph, index) => <p key={`${index}-${paragraph}`}>{paragraph}</p>)}<figure><img src="/about/bb-auto-exchange-exterior.png" alt="B & B Auto Exchange showroom with classic vehicles" /><figcaption>{story.imageCaption}</figcaption></figure><div className="about-license"><span className="about-license-icon">{credentialIcon ? <img src={credentialIcon} alt="" /> : 'OK'}</span><div><strong>{credential?.title ?? story.licenseTitle}</strong><p>{credential?.detail ?? story.licenseDetail}</p></div><b>{credential?.status ?? 'FOCUSED'}</b></div></article>
